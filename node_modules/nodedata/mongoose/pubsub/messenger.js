...
class Messenger extends events.EventEmitter {
    ...
    sendMessageToclient(client, repo, message, multiClients, collection) { }
    getAllUsersForNotification(entity) { return []; }
    ...
    ;
    sendPendingMessage(rechannel, lastemit, receiver) {
        var self = this;
        var query = { channel: rechannel, timestamp: { $gt: lastemit } };
        if (!this.Message) {
        ...
        stream.on('data', function data(doc) {
            ...
            if (self.subscribed[doc.channel]) {
                ...
                if (doc.message.singleWorkerOnRole || self.batchSize == 0) {
                    ...
                    // self.emit(doc.channel, doc.message);
                    if (!self.streamBuffer[doc.channel]) {
                        self.streamBuffer[doc.channel] = [];
                    }
