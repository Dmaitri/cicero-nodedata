"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
const decorators_1 = require("../../core/decorators");
const employee_1 = require('../models/employee');
const dynamic_repository_1 = require('../../core/dynamic/dynamic-repository');
let EmployeeRepository = class EmployeeRepository extends dynamic_repository_1.DynamicRepository {
};
EmployeeRepository = __decorate([
    decorators_1.repository({ path: 'employee', model: employee_1.employee }), 
    __metadata('design:paramtypes', [])
], EmployeeRepository);
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = EmployeeRepository;

//# sourceMappingURL=employeeRepository.js.map
