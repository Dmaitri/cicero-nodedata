"use strict";
const utils_1 = require("../metadata/utils");
const decorators_1 = require('../constants/decorators');
const decorators_2 = require('../constants/decorators');
const decorator_type_1 = require('../enums/decorator-type');
//export function repository(path: string, model: Function) {
function repository(params) {
    //params = params || <any>{};
    return function (target) {
        console.log('Repository - Path : ', params.path);
        target.prototype.path = params.path;
        target.prototype.model = params.model;
        if (params.exportType == undefined) {
            params.exportType = decorators_2.ExportTypes.REST;
        }
        utils_1.MetaUtils.addMetaData((target.prototype || target), {
            decorator: decorators_1.Decorators.REPOSITORY,
            decoratorType: decorator_type_1.DecoratorType.CLASS,
            params: params
        });
    };
}
exports.repository = repository;

//# sourceMappingURL=repository.js.map
