...
const initialize_messengers_1 = require('../initialize/initialize-messengers'); // name.role :{ role: string, accessmask: number, acl?: boolean }
        ...
        descriptor.value = function () {
            ...
            return mergeTask.apply(this, [args, originalMethod]).then(fullyQualifiedEntities => {
                ...
                let checkIfAClrequired = () => {
                    let user = principalContext_1.PrincipalContext.User;
                    if (user && user.getAuthorities() && initialize_messengers_1.allAutherizationRulesMap && initialize_messengers_1.allAutherizationRulesMap[this.path]) {
                        let isACL = true;
                        user.getAuthorities().forEach((curRole) => {
                            let aclRule = initialize_messengers_1.allAutherizationRulesMap[this.path][curRole];
                ...
                if (findActions.indexOf(originalMethod.name.toUpperCase()) >= 0) {
                    ...
                    let promiseOfAuthServerice = Q.when(true);
                    if (checkIfAClrequired()) {
                        promiseOfAuthServerice = post_filter_service_1.PostFilterService.postFilter(fullyQualifiedEntities, params);
                    }
...
function mergeTask(args, method) {
    ...
    switch (method.name.toUpperCase()) {
        ...
        case repo_actions_enum_1.RepoActions.findWhere.toUpperCase():
            ...
            break;
        case repo_actions_enum_1.RepoActions.findMany.toLocaleUpperCase():
            prom = rootRepo.findMany(args[0]).then((dbEntities) => {
                let mergedEntities = dbEntities.map(x => instance_service_1.InstanceService.getInstance(this.getEntity(), null, x));
                return mergeEntities(dbEntities, undefined, mergedEntities);
            });
            break;
    ...
    return prom.then(res => {
        ...
        if (res instanceof Array) {
            res.forEach(x => {
                res[constants_1.ConstantKeys.FullyLoaded] = true;
            });
        }
        else {
            res[constants_1.ConstantKeys.FullyLoaded] = true;
...
function mergeProperties(dbEntity, entity, mergedEntity) {
    ...
    if (tempMergedEntity && (tempMergedEntity instanceof Object && !(tempMergedEntity instanceof mongoose_1.Types.ObjectId))) {
    ...
    mergedEntity[constants_1.ConstantKeys.FullyLoaded] = true;
