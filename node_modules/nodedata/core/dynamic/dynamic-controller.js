...
class DynamicController {
    ...
    ensureLoggedIn(entity, action) {
        return function (req, res, next) {
            principalContext_1.PrincipalContext.getSession().run(function () {
                principalContext_1.PrincipalContext.save('write', 0);
    ...
    addRoutes() {
        ...
        exports_1.router.patch(this.path, this.ensureLoggedIn(this.entity, repo_actions_enum_1.RepoActions.patch), (req, res) => {
            ...
            console.log("hal model conversion end " + this.path);
            return this.repository.bulkPatch(req.body)
                .then((result) => {
                this.sendresult(req, res, result);
            }).catch(error => {
                console.log(error);
    ...
    invokeModelFunction(map, req, res, actions, hasFiles) {
        try {
            let modelRepo = {};
            if (this.repository.getEntityType() instanceof dynamic_repository_1.DynamicRepository) {
    ...
    searchFromDb(req, res, propertyKey) {
        var resourceName = this.getFullBaseUrlUsingRepo(req, this.repository.modelName());
        let resultquerydata = queryOptions_1.getQueryOptionsFromQuery(req.query);
    ...
    getHalModel1(model, resourceName, req, repo, noChildRecursion) {
        var selfUrl = {};
        selfUrl["href"] = resourceName; // + "/" + model._id;
        model["_links"] = {};
    ...
    sendresult(req, res, result) {
        if (principalContext_1.PrincipalContext && principalContext_1.PrincipalContext.get('write')) {
            console.log('Total number of writes before response:' + principalContext_1.PrincipalContext.get('write'));
        }
        if (principalContext_1.PrincipalContext && principalContext_1.PrincipalContext.get('cacheCount')) {
            console.log('Total number of cache hits before response:' + principalContext_1.PrincipalContext.get('cacheCount'));
        }
        res.set("Content-Type", "application/json");
        res.send(result);
    }
